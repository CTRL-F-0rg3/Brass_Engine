...
//  Obsługuje: Mesh rendering, Camera3D (perspective), Material, DirectionalLight
// =============================================================================
...
impl Mesh {
    /// Prosty sześcian jednostkowy.
    pub fn cube() -> Self {
        let v = |px: f32, py: f32, pz: f32, nx: f32, ny: f32, nz: f32, u: f32, v: f32| Vertex3D {
            position: [px, py, pz],
            normal:   [nx, ny, nz],
            uv:       [u, v],
        };
...
impl Default for Camera3D {
    fn default() -> Self {
        Self::new(Vec3::new(0.0, 3.0, 5.0), Vec3::ZERO)
    }
}
...
#[derive(Clone, Debug)]
pub struct Material {
    pub albedo:     Vec4,       // RGBA base color
...
impl Renderer3D {
    pub fn new(ctx: &RenderContext) -> Self {
        ...
        // ── Bind group layouts ────────────────────────────────────────────────
        let camera_bgl = bgl_uniform(device, "Camera3D BGL", ShaderStages::VERTEX_FRAGMENT);
        let model_bgl  = bgl_uniform(device, "Model BGL",    ShaderStages::VERTEX);
        let mat_bgl    = bgl_uniform(device, "Material BGL", ShaderStages::FRAGMENT);
        ...
        let layout = device.create_pipeline_layout(&PipelineLayoutDescriptor {
            ...
            bind_group_layouts:   &[&camera_bgl, &model_bgl, &mat_bgl, &light_bgl, &texture_bgl],
        ...
        let mat_data = MaterialUniform {
            albedo: [1.0; 4], metallic: 0.0, roughness: 0.8, _pad: [0.0; 2],
        };
        let material_buf = device.create_buffer_init(&util::BufferInitDescriptor {
            label:    Some("3D Mat Buf"),
            contents: bytemuck::bytes_of(&mat_data),
            usage:    BufferUsages::UNIFORM | BufferUsages::COPY_DST,
        });
        let material_bg = uniform_bg(device, "Mat BG", &mat_bgl, &material_buf);
        ...
        let light_buf = device.create_buffer_init(&util::BufferInitDescriptor {
            label:    Some("3D Light Buf"),
    ...
    /// Render wszystkich zebranych draw calls do podanego color view.
    pub fn render(
        &mut self,
        ctx:        &RenderContext,
        ...
        let light_data = LightUniform {
            direction: self.light.direction.to_array(),
            _p0:       0.0,
            color:     (self.light.color * self.light.intensity).to_array(),
        ...
        let mut encoder = ctx.device.create_command_encoder(
            &CommandEncoderDescriptor { label: Some("3D Encoder") }
        );

        {
            ...
            for call in &calls {
                let gpu_mesh = match self.meshes.get(&call.mesh_id) {
                    Some(m) => m,
                    None    => continue,
                };
                ...
                ctx.queue.write_buffer(&self.material_buf, 0, bytemuck::bytes_of(&mat_data));
                ...
                pass.set_bind_group(4, &tex.bind_group, &[]);
...
@group(4) @binding(0) var t_albedo:          texture_2d<f32>;
@group(4) @binding(1) var s_albedo:          sampler;
...
fn vs_main(in: VertIn) -> VertOut {
    ...
    // Normal matrix (uproszczona — zakłada uniform scale)
...
fn fs_main(in: VertOut) -> @location(0) vec4<f32> {
    ...
    let color      = ambient
                   + base_color.rgb * diffuse * light.color
                   + vec3<f32>(specular) * light.color;
